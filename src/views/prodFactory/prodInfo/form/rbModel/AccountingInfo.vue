            ...
            <template>
                <v-data-table :headers="headers" :items="projects" hide-actions class="elevation-1">
                    <template slot="items" slot-scope="props">
                        ...
                        <td class="text-xs-left">{{ props.item.accountSet }}</td>
...
<script>
import {getAccountingInfo} from '@/api/table';
export default {
    data () {
        return {
            headers: [
    ...
    methods: {
        queryDespositProdData() {
            getAccountingInfo().then(response => {
                this.projects = response.data.accountingInfo
            })
        }
    }
};
